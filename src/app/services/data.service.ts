...
export class DataService {
  ...
  getContacts(): Observable<Contact[]> {
    const contactsCollection = collection(this.firestore, 'libros');
  ...
  getContactById(id: string): Observable<Contact> {
    const document = doc(this.firestore, `libros/${id}`);
  ...
  createContact(contact: Contact): Promise<void> {
    const document = doc(collection(this.firestore, 'libros'));
    return setDoc(document, contact);
  }

  updateContact(contact: Contact): Promise<void> {
    const document = doc(this.firestore, 'libros', contact?.id);
